 ...
 * @return Remainder string which was not processed.
 */
string byBits(string sequence, int len) {
    while(len != 0) {
        int oldLen = sequence.length();
        sequence = parse(sequence);
        int diff = oldLen - sequence.length();
        len -= diff;
    }
    return sequence;
}
 ...
 * @return Remainder string which was not processed.
 */
string byPackets(string sequence, int count) {
 ...
 * @return Remainder string which was not processed.
 */
string op(string payload) {
    char lti = payload[0];
    payload = payload.substr(1,string::npos);
    string remainder;
    if(lti == '1') {
        int numSubPackets = binStrToInt(payload.substr(0, 11));
        payload = payload.substr(11, string::npos);
        remainder = byPackets(payload, numSubPackets);
    }else{
        int bitsSubPackets = binStrToInt(payload.substr(0, 15));
        remainder = payload.substr(15+bitsSubPackets, string::npos);
        payload = payload.substr(15, bitsSubPackets);
        byBits(payload, bitsSubPackets);
    }
    return remainder;
}
...
string parse(string packet) {
    string version = packet.substr(0, 3);
    string type = packet.substr(3,3);
    string payload = packet.substr(6, string::npos);
    versionSum += stoi(version, 0, 2);
    if(type == "100") {
        packet = literal(payload);
    }else {
        packet = op(payload);
    }
    return packet;
